...
*     R12 = MIN(R12,R23)
*     IF (R12.LT.SEMI) THEN
*     WRITE (6,125)  TNOW, IPN, NPERT, ENERGY, GPERT, DEGR,
*    &               (1.0/RINV(K),K=1,N-1)
* 125 FORMAT (' CHAIN!   T IPN NP ENER G DEGR R  ',
*    &                   F13.7,2I4,F12.8,1P,6E10.2)
*     CALL FLUSH(6)
*     END IF
*       Check movie output.
...
*       Include extra BH information at late stages (when CALL REDUCE rare).
      ...
      END IF
*
*       Predict perturbers & XC, UC and form new LISTC every 5 steps.
      IF (MOD(NSTEP1,2).EQ.0) THEN
          JJ = 0
          CALL XCPRED(2)
          CALL CHLIST(JJ)
      ELSE
...
*       Allow extreme cases (GPERT > 0.1) only.
      IF (GPERT.GT.0.10.AND.N.LT.5.AND.NSTEP1.GT.NEXT) INJ = -1
...
*       Include extra diagnostics during late GR stages (CALL REDUCE rare).
      IF (IPN.GE.2.AND.MOD(NSTEP1,100).EQ.0) THEN
          WRITE (6,147)  TNOW, NPERT, IPN, ECC, SEMI, TZ, DW
  147     FORMAT (' INSPIRAL    T NP IPN E A TZ DW ',
...
*       Include 10% tolerance to ensure collision condition near peri.
  ...
  157     CONTINUE
     ...
     &                       NSTEP1
  159         FORMAT (' CHAIN COLLISION(?)    NAMC ECC PM A/RSUM RC ',
     &                                        2I6,F9.5,1P,2E10.2,0P,I6)
              CALL REDUCE(I1,I2,ISUB)
              GO TO 258
          END IF
      END IF
*
...
*       Include net energy loss via ECOLL.
          CALL DECORR(DE)
          WRITE (6,180)  ENERGY, ENER0, DE, ECH, DEGR
  180     FORMAT (' CHAIN CHECK    ENERGY ENER0 DE ECH DEGR ',
     &                             1P,5E12.4)
...
*       Give priority to CHAIN ESCAPE cases.
           IF (KCASE.LE.-3) THEN
              ...
              ECH = ENERGY - DEGR
*       Continue integration if remaining time is positive.
              IF (TMAX-TIMEC.LT.0.0) GO TO 400
              GO TO 30
           END IF
*
...
*       Include termination condition for N = 2 or large perturbation.
     ...
     &    (ENERGY.GT.0.0.AND.(RSUM.GT.20.0*RGRAV.OR.GPERT.GT.0.05).AND.
     &    NSTEP1.GT.10)) THEN
