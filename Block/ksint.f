...
*       Skip outward motion or separation > 5*RMIN but allow NCH = 2.
     ...
     &                GAMMA(IPAIR).LT.1.0D-03) GO TO 100
  ...
  220                 FORMAT (' ACTIVATE CHAIN    T JCL K* NP G A R ',
     &                                            F9.1,I7,3I4,1P,3E10.2)
                      KSPAIR = IPAIR
                  ELSE IF (SEMI.LT.0.1*RMIN.AND.DW.GT.1.0D-03) THEN
                      WRITE (6,222)  TIME+TOFF, SEMI, R(IPAIR), DW
  222                 FORMAT (' ENFORCE CHAIN    T A R DW ',
     &                                           F12.4,1P,3E10.2)
                      KSPAIR = IPAIR
                  ELSE
                      GO TO 100
...
*       Include convergence criterion DH = H'*DTU + H''*DTU**2/2 = 0.001*|H|.
      IF (GI.GT.1.0D-05) THEN
          ...
          DTU = MIN(DTU2,DTU)
      END IF
*     IF (KSTAR(I1).EQ.14.AND.KSTAR(I2).EQ.14)  DTU = 0.25*DTU
*
